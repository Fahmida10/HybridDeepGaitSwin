...
def evaluate_HID2022(data, dataset, metric='euc'):
    """Rank-1 on HID 2022 training subjects (gallery=first seq, probe=remaining seqs)."""
    from collections import defaultdict

    msg_mgr = get_msg_mgr()
    msg_mgr.log_info("Evaluating HID 2022 train split (Rank-1)")
    feature, label, seq_type = data['embeddings'], data['labels'], data['types']
    label = np.array(label)
    seq_type = np.array(seq_type)

    subject_seqs = defaultdict(list)
    for i, (lab, st) in enumerate(zip(label, seq_type)):
        if not (str(lab).isdigit() and int(lab) < 500):
            continue
        subject_seqs[str(lab)].append((st, i))

    gallery_idx, probe_idx, probe_labels = [], [], []
    for lab in sorted(subject_seqs.keys()):
        seqs = sorted(subject_seqs[lab], key=lambda x: x[0])
        if len(seqs) < 2:
            continue
        gallery_idx.append(seqs[0][1])
        for _, idx in seqs[1:]:
            probe_idx.append(idx)
            probe_labels.append(lab)

    if not probe_idx:
        msg_mgr.log_info("No probe sequences found for HID Rank-1 evaluation.")
        return {}

    gallery_x = feature[gallery_idx, :]
    probe_x = feature[probe_idx, :]
    gallery_y = np.array([str(label[i]) for i in gallery_idx])
    probe_y = np.array(probe_labels)

    dist = cuda_dist(probe_x, gallery_x, metric)
    idx = dist.cpu().sort(1)[1].numpy()
    acc = np.round(
        np.sum(np.cumsum(np.reshape(probe_y, [-1, 1]) == gallery_y[idx[:, 0:1]], 1) > 0, 0)
        * 100 / dist.shape[0], 2)
    rank1 = float(np.mean(acc[0]))
    msg_mgr.log_info('=== HID 2022 Train Split (Gallery=seq[0], Probe=seq[1:]) ===')
    msg_mgr.log_info('Rank-1: %.2f%%' % rank1)
    msg_mgr.log_info('Gallery: %d sequences | Probe: %d sequences' % (len(gallery_idx), len(probe_idx)))
    return {"scalar/test_accuracy/Rank-1": rank1}
