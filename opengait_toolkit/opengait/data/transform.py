...
class ResizeSilTransform():
    """
    Resize silhouette sequences to a fixed (H, W).

    This is deterministic preprocessing and avoids resolution mismatch (e.g. 64x64 vs 128x128).
    It only resizes when current size differs from target.
    ...
    def __init__(self, img_h=128, img_w=128, interpolation="nearest"):
    ...
    def __call__(self, x):
        ...
        if len(x.shape) == 2:
            ...
            if (h, w) == (self.img_h, self.img_w):
                return x
            inter = cv2.INTER_NEAREST if self.interpolation == "nearest" else cv2.INTER_LINEAR
            return cv2.resize(x, (self.img_w, self.img_h), interpolation=inter)
        if len(x.shape) == 3:
            t, h, w = x.shape
            if (h, w) == (self.img_h, self.img_w):
                return x
            inter = cv2.INTER_NEAREST if self.interpolation == "nearest" else cv2.INTER_LINEAR
            # Resize per frame to keep memory small.
            out = [cv2.resize(x[i], (self.img_w, self.img_h), interpolation=inter) for i in range(t)]
