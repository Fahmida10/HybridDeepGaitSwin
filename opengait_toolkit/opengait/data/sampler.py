...
import random
class BalancedTripletSampler(tordata.sampler.Sampler):
    """
    Dataset-balanced TripletSampler for multi-dataset training.
    Distributes batch_size[0] identities equally across source datasets,
    identified by the prefix before the first '_' in each label.
    """
    ...
    def __init__(self, dataset, batch_size, batch_shuffle=False):
        ...
        for label in dataset.label_set:
            prefix = label.split("_")[0] if "_" in label else "OTHER"
            self.dataset_groups[prefix].append(label)
        self.group_names = sorted(self.dataset_groups.keys())
        if self.rank == 0:
            print(f"[BalancedTripletSampler] {len(self.group_names)} groups:")
            for g in self.group_names:
                print(f"  {g}: {len(self.dataset_groups[g])} identities")
    ...
    def __iter__(self):
        ...
        while True:
            base = P // n_groups
            remainder = P - base * n_groups
            extra = sync_random_sample_list(
                self.group_names, remainder) if remainder > 0 else []

            pid_list = []
            for g in self.group_names:
                n = base + (1 if g in extra else 0)
                if n == 0:
                    continue
                pid_list.extend(
                    sync_random_sample_list(self.dataset_groups[g], n))

            sample_indices = []
            for pid in pid_list:
                indices = self.dataset.indices_dict[pid]
                sample_indices += sync_random_sample_list(indices, k=K)
            ...
            total_size = int(math.ceil(
                total_batch_size / self.world_size)) * self.world_size
            sample_indices += sample_indices[:(
                total_batch_size - len(sample_indices))]
            sample_indices = sample_indices[
