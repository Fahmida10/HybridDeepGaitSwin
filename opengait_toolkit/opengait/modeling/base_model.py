...
class BaseModel(MetaModel, nn.Module):
    ...
    def save_ckpt(self, iteration):
        if torch.distributed.get_rank() == 0:
            ...
            checkpoint = {
                ...
                'iteration': iteration}
            new_ckpt_path = osp.join(self.save_path, 'checkpoints/{}-{:0>8}.pt'.format(save_name, iteration))
    ...
    def _load_ckpt(self, save_name):
        ...
        if not load_ckpt_strict:
            ...
            self.msg_mgr.log_info(sorted(filtered_state.keys()))
            if mismatched:
                self.msg_mgr.log_warning("Skip shape-mismatched params: %s" % mismatched)
    ...
    def resume_ckpt(self, restore_hint):
        if isinstance(restore_hint, int):
            save_name = self.engine_cfg['save_name']
            ckpt_8 = osp.join(self.save_path, 'checkpoints/{}-{:0>8}.pt'.format(save_name, restore_hint))
            ckpt_5 = osp.join(self.save_path, 'checkpoints/{}-{:0>5}.pt'.format(save_name, restore_hint))
            save_name = ckpt_8 if osp.exists(ckpt_8) else ckpt_5
